    ...
    <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
        @if($type === 'request_received')
            ...
            @if($bookRequest->message)
                ...
                <p style="background: white; padding: 15px; border-radius: 8px; font-style: italic;">{{ $bookRequest->message }}</p>
            @endif
        ...
        @elseif($type === 'request_approved')
            ...
            <p>Great news! Your request to borrow <strong>{{ $bookRequest->book->title }}</strong> has been approved.</p>
        ...
        @elseif($type === 'request_rejected')
            ...
            <p>Hello {{ $bookRequest->borrower->name }},</p>
            <p>Unfortunately, your request to borrow <strong>{{ $bookRequest->book->title }}</strong> has been rejected by the owner.</p>
        ...
        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; text-align: center;">
            <a href="{{ route('requests.index') }}" style="display: inline-block; background: #667eea; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">View Requests</a>
        </div>
    </div>
